...
class BinanceFuturesFeed:
    ...
    def _on_depth(self, message):
        ...
        U = message["U"]
        u = message["u"]
        pu = message["pu"] #futures
        last_id = book.last_update_id

        # print('U:', U, 'last_id:', last_id, 'u:', u, 'pu:', pu)
...
class MarketMakingStrategy:
    ...
    def generate_quotes(self, state): ############## Fair Value Anchoring (not market anchoring)
        ...
        # 5. round to tick
        # bid = self.config.round_price(bid) # only round at order stage
...
class LiveExecution:
    ...
    def __init__(self, config, state, broker, recorder, params):
        ...
        self.base_size = self.params["base_size"]
    ...
    def _compute_order_size(self, signal): # Asymmetrical Quoting
        ...
        base_size = self.base_size # 0.5 because demo margin is max 1.0 for maintenance margin 1.0
    ...
    def place_market(self):
        ...
        bid_resp = self.broker.place_market(client_oid, pos, side, ts)
        order.resp = bid_resp

        # print(f'ts: {ts} - MARKET ORDER with order_id: client_oid {client_oid}')
    ...
    def place_quotes(self, signal):
        ...
        bid_change = (
            self.last_bid is None or
            abs(desired_bid - self.last_bid) >= tick
        )

        ask_change = (
            self.last_ask is None or
            abs(desired_ask - self.last_ask) >= tick
        )
        ...
        if bid_order is None:
            try:
                ...
                bid_resp = self.broker.place_limit("BUY", price=desired_bid, size=order_sizes["bid_size"], client_oid=client_oid, ts=ts)
                order.resp = bid_resp
            ...
            bid_resp = self.broker.cancel_order(bid_order, ts=ts)
        ...
        if ask_order is None:
            try:
                ...
                ask_resp = self.broker.place_limit("SELL", price=desired_ask, size=order_sizes["ask_size"], client_oid=client_oid, ts=ts)
                order.resp = ask_resp
            ...
            ask_resp = self.broker.cancel_order(ask_order, ts=ts)
    ...
    def process_trade(self, trade):
        ...
        self.state.last_fill_candidate = orders[0]
...
class BinanceUserStream:
    def __init__(self, config, state, execution, broker, recorder):
        ...
        self.connected = threading.Event()   # <-- ADD THIS
    ...
    def on_message(self, ws, msg):
        ...
        if exec_type == "NEW":
            ...
            if side == "BUY": # update last bid here for confirmed live orders
                ...
                self.state.my_queue_position["bids"][order.price] = book_size
                ...
                book_size = self.state.market_book.asks.get(order.price, 0.0)

                # record estimated queue position
                self.state.my_queue_position["asks"][order.price] = book_size
                order.queue_ahead_at_join = book_size
        ...
        # -------------------------
            ...
            if side == 'BUY':
                self.state.reset_queue_ahead("bids", order.price) # reset queue position and queue flow
            
            else:
                self.state.reset_queue_ahead("asks", order.price) # reset queue position and queue flow
...
if __name__ == "__main__":
    ...
    params = load_manifest(r"D:\OneDrive\Trading\Market Making\data\manifest_live.json")
