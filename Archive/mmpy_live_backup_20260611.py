...
class BinanceFeed:
    ...
    def start(self):
        ...
        depth_socket = f"wss://stream.binancefuture.com/ws/{self.instrument}@depth@100ms" # testnet futures
        ...
        # time.sleep(1.0) no wait for futures 
        ...
        with self.buffer_lock:
            startup_buffer = list(self.depth_buffer)

        startup_buffer.sort(key=lambda x: x["U"])

        for msg in startup_buffer:
        ...
        #for msg in self.depth_buffer:
            ...
            if not synced:
                ...
                if U <= last_update_id + 1 <= u: # official bridge connection
            ...
            if U > self.state.market_book.last_update_id + 1:
                print(
                    f"GAP DETECTED "
                    f"last_id={self.state.market_book.last_update_id} "
                    f"U={U} "
                    f"u={u}"
                )
        ...
        # LIVE MODE
        # -------------------------
        with self.buffer_lock: # process anything that arrived while syncing
            late_buffer = self.depth_buffer[len(startup_buffer):]
        
        for msg in late_buffer:

            U = msg["U"]
            u = msg["u"]

            if u <= self.state.market_book.last_update_id:
                continue

            if U > self.state.market_book.last_update_id + 1:
                raise RuntimeError(
                    f"LATE BUFFER GAP "
                    f"last={self.state.market_book.last_update_id} "
                    f"U={U} "
                    f"u={u}"
                )
