...
public:
    ...
    BinanceSpotFeed(State& state, MarketConfig& config, function<void()> on_market_data,
                    function<void(const Trade&)> on_trade_event, const json& params):
                    state(state), config(config), on_market_data(move(on_market_data)),
...
public:
    ...
    BinanceFuturesFeed(State& state, MarketConfig& config, function<void()> on_market_data,
                    function<void(const Trade&)> on_trade_event, const json& params):
                    state(state), config(config), on_market_data(move(on_market_data)),
                    on_trade_event(move(on_trade_event)),
                    instrument(params["instrument"]) {}
...
struct Snapshot {
    ...
    struct {
        ...
        string buy_order;
        string sell_order;
        string last_fill_candidate;
        string last_order_update;
    } execution;
    ...
    struct {
        string time;
        string last_trade_ts;
        string last_depth_ts;
    } system;
};
...
public:
    ...
    Engine(MarketConfig& config, State& state, MarketMakingStrategy& strategy, Execution& execution,
            DatasetRecorder& recorder, Dashboard* dashboard, const json& params, queue<Signal>& signal_queue)
        : config(config), params(params), state(state), strategy(strategy), execution(execution),
        recorder(recorder), dashboard(dashboard), signal_queue(signal_queue)
    {
        struct_model = params["models"]["struct_model"];
    ...
    string tradeToString(const Trade& trade){}
        return trade ? format("{:<5} | {:>10.4f} | {:>8.6f}", trade.side, trade.price, trade.qty) : "—";
    }

    string orderToString(const Order* order){
        return order ? format("{:<5} | {:>10.4f} | {:>8.6f} [{}]", 
            order->side, config.from_tick(order->price), order->qty, order->status) : "—";
    }
    ...
    Element centered_inventory_bar(const double& inv, const double max_inv = 10.0, const int width = 21){
        ...
        if(scaled > 0){
            for(int i = 0; i < scaled; ++i) right[i] = '█';
        }
        else if(scaled < 0){
            for(int i = 0; i < -scaled; ++i) left[half - 1 - i] = '█';
        }
        else return hbox({text(left) | color(Color::Red), text("|") | 
            color(Color::White), text(right) | color(Color::Green)});
    }

    Component Dashboard(SnapshotStore& store) {

        return Renderer([&]{
            Snapshot snap = store.get();

            auto market = vbox({
                text("MARKET" | bold | color(Color::Yellow)),
                text("Mid" + format("{:<15.4f}", snap.market.mid)),
                text("Spread" + format("{:<15.4f}", snap.market.spread)),
                text("Best Bid / Size" + format("{:<10.4f}", snap.market.bid) + " (" + format("{:<6.4f}", snap.market.bid_size) + ")"),
                text("Best Ask / Size" + format("{:<10.4f}", snap.market.ask) + " (" + format("{:<6.4f}", snap.market.bid_size) + ")"),
                text("EWMA Vol" + format("{:.2e}", snap.market.ewma_vol)),
                text("Order Imbalance" + format("{:<15.4f}", snap.market.order_imbalance)),
                text("Trade Imbalance" + format("{:<15.4f}", snap.market.trade_imbalance)),
                text("Last Trade" + snap.market.trade),
            });

            auto regime = vbox({
                text("REGIME") | bold | color(Color::Yellow),
                text("Regime: " + snap.regime.regime),
                text("Confidence: " + format("{:<15.2f}", snap.regime.confidence)),
            });


            auto signals = vbox({
                text("SIGNALS" | bold | color(Color::Yellow)),
                text("Fair Value" + format("{:<15.4f}", snap.signals.fair_value)),
                text("Inventory Skew" + format("{:<15.4f}", snap.signals.skew)),
                text("Reservation" + format("{:<15.4f}", snap.signals.reservation)),
                text("Alpha Struct" + format("{:<15.2f}", snap.signals.alpha_struct)),
                text("ML Signal Quality" + format("{:<15.2f}", snap.signals.signal_quality)),
            });
            
            auto quotes = vbox({
                text("QUOTES" | bold | color(Color::Yellow)),
                text("My Bid / Size" + format("{:<10.4f}", snap.quotes.my_bid) + " (" + format("{:<6.4f}", snap.quotes.current_bid_size) + ")"),
                text("My Ask / Size" + format("{:<10.4f}", snap.quotes.my_ask) + " (" + format("{:<6.4f}", snap.quotes.current_ask_size) + ")"),
            });

            auto execution = vbox({
                text("EXECUTION" | bold | color(Color::Yellow)),
                text("Queue Ahead / Bid" + format("{:<10.4f}", snap.mid.bid) + " (" + format("{:<6.4f}", snap.execution.bid_queue) + ")"),
                text("Queue Ahead / Ask" + format("{:<10.4f}", snap.mid.ask) + " (" + format("{:<6.4f}", snap.execution.ask_queue) + ")"),
                text("Queue Pressure / Bid" + format("{:<10.4f}", snap.mid.bid) + " (" + format("{:<6.4f}", snap.execution.bid_pressure) + ")"),
                text("Queue Pressure / Ask" + format("{:<10.4f}", snap.mid.ask) + " (" + format("{:<6.4f}", snap.execution.ask_pressure) + ")"),
                text("Open Orders" + snap.execution.buy_order),
                text("" + snap.execution.sell_order),
                text("Last Fill Candidate" + snap.execution.last_fill_candidate),
                text("Last Order Update" + snap.execution.last_order_update),
            });

            auto risk = vbox({
                text("RISK" | bold | color(Color::Yellow)),
                hbox({text("Inventory"), color_risk(snap.risk.inventory)})
                hbox({text("Realized PnL") + color_pnl(snap.risk.realized_pnl)}),
                hbox({text("Unrealized PnL") + color_pnl(snap.risk.unrealized_pnl)}),
                hbox({text("Fees Paid") + color_pnl(snap.risk.fees_paid)}),
                hbox({text("Total PnL") + color_pnl(snap.risk.total_pnl)}),
                hbox({text("Risk") + centered_inventory_bar(snap.risk.inventory)}),
            });

            auto system = vbox({
                text("SYSTEM" | bold | color(Color::Yellow)),
                text("Time" + snap.system.time),
                text("Last Trade ts" + snap.system.last_trade_ts),
                text("Last Depth ts" + snap.system.last_depth_ts),
            });

            return vbox({
                text(snap.title.struct_model + " | " + snap.title.mode + " | " + snap.title.exchange
                    + " | " + snap.title.instrument + " | " + snap.title.regime + " | " 
                    + snap.title.pnl_pct > 0 ? "+" : "" + format("{:.2f}", snap.title.pnl_pct)) | bold | center,
                separator(),
                hbox({ market, separator(), regime, separator(), 
                        signals, separator(), quotes, separator(), execution, 
                        separator(), risk, separator(), system})
            });
        });
    };
...
public:
    ...
    unordered_map<string, unordered_map<int64_t, double> queue_flow;
    unordered_map<string, unordered_map<int64_t, double> my_queue_position;
    unordered_map<double, deque<TradeEvent>> trade_buckets;
    double window_ms = 1000;

    double last_trade_ts = 0;
    Trade last_trade;
    double last_depth_ts = 0;
    Order* last_fill_candidate;
    Order* last_order_update;
    ...
    double get_vol() return sqrt(ewma_var);
    ...
    void on_fill(const double& price, const double& qty, const string& side, const string& liquidity){
    ...
    double get_value(auto& m, const string& side, const int64_t& price){
    ...
    double compute_queue_ahead(const string& side, const int64_t& price){
        double my_pos = get_value(my_queue_position, side, price);
        double flow   = get_value(queue_flow, side, price);

        double ahead = my_pos - flow;
        return max(0.0, ahead);
    }

    void reset_queue_ahead(const string& side, const int64_t& price){
        my_queue_position[side].erase(price);
        queue_flow[side].erase(price);
    }
