...
#include "mmpy_snapshot.hpp" //struct snapshot
...
public:
    ...
    pair<uint64_t, json> set_orderbook_snapshot(const json& snapshot){
...
public:
    ...
    vector<std_string> depth_buffer;
    ...
    BinanceSpotFeed(MarketConfig& config, State& state, function<void()> on_market_data,
                    ...
                    log_orderbook_snapshot(move(log_orderbook_snapshot)), instrument(params["instrument"]) {
                        instrument_upper = ranges::transform(instrument, instrument.begin(), [](unsigned char c){ return toupper(c);});
                    }
    ...
    void depth_loop(){
        ...
        while(running){
            ws.read(buffer);
            std_string msg = boost::beast::buffers_to_string(buffer.data());
    ...
    void trade_loop(){
        ...
        while(running){
            ...
            trade.price = stod(doc["p"].get_ref<const std_string&>());
            trade.qty   = stod(doc["q"].get_ref<const std_string&>());
...
public:
    ...
    BinanceFuturesFeed(MarketConfig& config, State& state, function<void()> on_market_data,
                    ...
                    on_trade_event(move(on_trade_event)), log_event(move(log_event)),
                    log_orderbook_snapshot(move(log_orderbook_snapshot)), instrument(params["instrument"]) {
                        ranges::transform(instrument, instrument.begin(), [](unsigned char c){ return toupper(c);});
                    }
...
public:
    ...
    vector<json> events;
    const json& orderbook_snapshot;

    std_string events_path;
    std_string snapshot_path;
                    ...
                    log_orderbook_snapshot(move(log_orderbook_snapshot))
    {
        ...
        snapshot_path = params["folder_path"].get<std_string>() + "/" +
    ...
    void load_events(){
        ...
        sort(events.begin(), events.end(), [](const auto& a, const auto& b) {return a["ts"] < b["ts"];});
    }
    ...
    void start() override {
        ...
        auto [snapshot_id, snapshot] = state.market_book.set_orderbook_snapshot(orderbook_snapshot);

        state.initialized = true;
        log_orderbook_snapshot(orderbook_snapshot);

        cout << "REPLAY SOCKETS STARTED\n";
    ...
    void stop() override {
        cout << "TOPPING BINANCE REPLAY FEED\n";
        running = false;
        cout << "BINANCE FEED REPLAY STOPPED\n";
    }
    ...
    void run(){
        while(running && i < events.size()){
            ...
            if(last_ts.has_value()){
                ...
                this_thread::sleep_for(duration<double>(max(0.0, dt / speed_multiplier)));
            }
    ...
    void process_event(const json& event){
        std_string type = event["type"];

        if(type == "depth") on_depth_message(event["message"]);
        else if(type == "trade") on_trade_message(event["message"]);
    }
...
public:
    ...
    std_string tradeToString(const Trade& trade){
...
public:
   ...
   TradingSystem(const json& params) : 
                params(params), config(params), state(config, params), strategy(config, params), 
                recorder(config, state, params), dashboard_terminal(snapshot_store), dashboard_server(params) {initialize();}

    void initialize(){
        if(params["mode"].get<std_string>() == "live"){
            broker = BinanceBroker(config, params);
        ...
        else execution = make_unique<PaperExecution>(config, state, recorder, params);
        ...
        auto log_ev = [this](const std_string& type, uint64_t ts, const std_string& message) {
            recorder.log_event(type, ts, message);};
    ...
    void start_dashboard_loop(){
        threads.emplace_back([this](){
            while(dashboard_running){
                ...
                json msg = {
                    {"type","snapshot"},
                    {"data", dashboard_server.snapshot_to_json(snap)}
                };

                dashboard_server.publish(msg);
