...
class BinanceFuturesFeed:
    def __init__(self, state, on_market_data, on_trade_event, logger, params):
        ...
        self.depth_msgs = deque(maxlen=10000)
    ...
    def start(self):
        ...
        depth_socket = f"wss://stream.binancefuture.com/ws/{self.instrument}@depth@100ms" # testnnet
        ...
        # -------------------------
        while self.depth_msgs:
            msg = self.depth_msgs.popleft()
            ...
            if u < snapshot_id:
                print('old u')
                print('BUFFER" U:', U, 'snapshot_id:', snapshot_id, 'u:', u)
                continue
...
class LiveExecution:
    ...
    def place_quotes(self, signal):
        ...
        bid_change = (
            self.last_live_bid is None or
            abs(desired_bid - self.last_live_bid) >= tick
        )

        ask_change = (
            self.last_live_ask is None or
            abs(desired_ask - self.last_live_ask) >= tick
        )
        ...
        if bid_order is None:
            ...
            book_size = self.state.market_book.bids.get(self.config.to_tick(desired_bid), 0.0)

            # record estimated queue position
            self.state.my_queue_position["bids"][self.config.to_tick(desired_bid)] = book_size
        ...
        if ask_order is None:
            ...
            book_size = self.state.market_book.asks.get(self.config.to_tick(desired_ask), 0.0)

            # record estimated queue position
            self.state.my_queue_position["asks"][self.config.to_tick(desired_ask)] = book_size
...
class TradingSystem:
    def __init__(self, params):
        ...
        # Market Data Layer
        FEEDS = {
            ("live", "binance_futures"): BinanceFuturesFeed,
