...
public:
    ...
    RegimeModel(const json& artifact){
        ...
        cout << "INITIALIZED: " << model_name << ", target: " << target << ", D: " << D << "\n";
    }

    void pack_features(const Regime& regime){
        x_input[0] = regime.spread;
        x_input[1] = regime.volatility;
        x_input[2] = regime.order_imbalance;
        x_input[3] = regime.trade_imbalance;
        x_input[4] = regime.microprice_dev;
        // x_input[5] = regime.quote_churn;
    }
...
public:
    ...
    void pack_features(const Features& f){
        x_input[0] = f.spread;
        x_input[1] = f.volatility;
        x_input[2] = f.order_imbalance;
        x_input[3] = f.trade_imbalance;
        x_input[4] = f.microprice_dev;
    }
...
public:
    ...
    void pack_features(const Features& f){
        ...
        x_input[3] = f.trade_imbalance;
        x_input[4] = f.microprice_dev;
        x_input[5] = (f.regime_id == 0) ? 1.0f : 0.0f;
        x_input[6] = (f.regime_id == 1) ? 1.0f : 0.0f;
        x_input[7] = (f.regime_id == 2) ? 1.0f : 0.0f;
    }
...
public:
    ...
    pair<Toxicity, double> compute_toxicity(State& state, const Features& features){
        
        Toxicity toxicity;

        if(toxicity_model == nullptr) return {toxicity, 0.0};

        double prediction = toxicity_model->predict(features);
        double toxicity_signal_quality = compute_signal_quality(state.mfs.toxicity_signal_log);
        ...
        toxicity.horizon_ms = toxicity_model->horizon_ms;
        toxicity.pred = prediction;

        return {toxicity, toxicity_signal_quality};
    }

    double compute_spread(const Features& features, const Policy& policy, const Toxicity& toxicity){

        double vol_component = 3.0 * features.volatility * features.mid; // added dimension change to mid price
        double raw_spread = max(features.spread, vol_component);

        // double spread = raw_spread * policy.spread_multiplier * (1.0 + toxicity.k_spread * toxicity.tox);

        // test for base_spread
        double spread = config.base_spread * features.spread * policy.spread_multiplier * (1.0 + toxicity.k_spread * toxicity.tox);
    ...
    Signal generate_quotes(State& state){ // put execution here in the mean time
        ...
        auto [toxicity, toxicity_signal_quality] = compute_toxicity(state, features);
        ...
        signal.toxicity_signal_quality = toxicity_signal_quality;
