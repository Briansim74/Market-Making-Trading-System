...
public:
    ...
    shared_ptr<Order> get_order(const std_string& msg_client_oid) override {
        lock_guard<mutex> lock(orders_mtx);
...
public:
    ...
    void on_message(const std_string& msg){
        ...
        shared_ptr<Order> order = execution.get_order(client_oid);
        ...
        order->exchange_ts = ts;
        order->ack_latency_ms = ts - order->ts;
