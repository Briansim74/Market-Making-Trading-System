...
class OrderBook:
    ...
    def apply_snapshot(self, snapshot):
        with self.lock:
            ...
            self.bids = SortedDict({
                int(round(float(p) / self.tick_size)): float(q)
    ...
    def apply_delta(self, bids, asks):
        with self.lock:
            for p, q in bids:
                p = int(round(float(p) / self.tick_size))
                q = float(q)
                if q == 0:
    ...
    def set_last_update(self, new_id):
...
class BinanceFeed: # FeedClass(
    def __init__(self, state, on_market_data, on_trade_event, logger, params):
        ...
        self.symbol = params["instrument"]
        self.on_update = None

        self.book = self.state.market_book
    ...
    def start(self):
        ...
        ws_url = f"wss://stream.binancefuture.com/ws/{self.symbol}@depth@100ms"
        ...
        # allow stream warmup (CRITICAL)
        time.sleep(0.5)

        # snapshot AFTER WS is already running
        self.snapshot = self._fetch_snapshot()
        self.book.apply_snapshot(self.snapshot)

        last_id = self.book.last_update_id

        print("SNAPSHOT:", last_id)
        ...
        buffer.sort(key=lambda x: x["U"])

        # find first usable event (RELAXED RULE)
        synced = False
        start_index = None
        ...
        if not synced:
            print("NO COVERAGE → retry sync (not crash)")
            return self._retry()

        # apply from first valid point onward
        for msg in buffer[start_index:]:
            last_id = self._apply(msg, last_id)

        self.book.last_update_id = last_id

        self.state = "LIVE"
        print("SYNC COMPLETE → LIVE")
    ...
    def _apply(self, msg, last_id):
        ...
        if U > last_id + 1:
            raise RuntimeError(f"GAP DETECTED {last_id} -> {U}")

        bids = [(p, q) for p, q in msg["b"]]
        asks = [(p, q) for p, q in msg["a"]]
