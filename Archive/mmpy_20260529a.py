...
# Feed Layer
class BinanceFeed:
    def __init__(self, state, instrument, on_market_data, on_trade_event, logger):
        self.state = state
...
# Orchestration Layer
class Engine:
    def __init__(self, config, state, strategy, execution, recorder, model, mode, exchange, instrument, dashboard):
        self.config = config
        self.state = state
    ...
    def build_snapshot(self): # FOR REACT
        state = self.state
        book = state.market_book
        execution = self.execution
    ...
    def render_dashboard(self):
        ...
        title=f"{self.model} | {self.mode} | {self.exchange} | {self.instrument} | pnl={pnl_pct_sign}{pnl_pct:.4f}%"
        ...
        table.add_row(
            "Mid",
            f"{snapshot["market"]["mid"]:<15.4f}"
        )

        table.add_row(
            "Spread",
            f"{snapshot["market"]["spread"]:<15.4f}"
        )

        table.add_row(
            "Best Bid / Size",
            f"{snapshot["market"]["bid"]:<10.4f} ({snapshot["market"]["bid_size"]:<6.4f})"
        )

        table.add_row(
            "Best Ask / Size",
            f"{snapshot["market"]["ask"]:<10.4f} ({snapshot["market"]["ask_size"]:<6.4f})"
        )

        table.add_row(
            "EWMA Vol",
            f"{snapshot["market"]["ewma_vol"]:.2e}"
        )

        table.add_row(
            "Order Imbalance",
            f"{snapshot["market"]["order_imbalance"]:<15.4f}"
        )

        table.add_row(
            "Trade Imbalance",
            f"{snapshot["market"]["trade_imbalance"]:<15.4f}"
        )

        table.add_row(
            "Last Trade",
            snapshot["market"]["trade"]
        )
        ...
        table.add_row(
            "Queue Ahead / Bid",
            f"{snapshot["bid"]:<10.4f} ({bid_queue:<6.4f})"
        )

        table.add_row(
            "Queue Ahead / Ask",
            f"{snapshot["ask"]:<10.4f} ({ask_queue:<6.4f})"
        )

        table.add_row(
            "Queue Pressure / Bid",
            f"{snapshot["bid"]:<10.4f} ({bid_pressure:<6.4f})"
        )

        table.add_row(
            "Queue Pressure / Ask",
            f"{snapshot["ask"]:<10.4f} ({ask_pressure:<6.4f})"
        )
        ...
        inv_bar = self.centered_inventory_bar(inv=snapshot["inventory"], max_inv=10, width=21)
...
class ReplayFeed:
    ...
    def __init__(self, state, on_market_data, on_trade_event, logger, replay_events):

        self.events = replay_events["events"]
        self.orderbook_snapshot = replay_events["orderbook_snapshot"]
        self.state = state
...
class TradingSystem:
    def __init__(self, params):
        ...
        self.replay_events = params["files"]["replay_events"]
        self.server_config = params["server_config"]
        self.tick_size = params["tick_size"]

        # === Monitoring Layer ===
        self.dashboard = DashboardServer(server_config=self.server_config)
        
        # === Market Configuration ===
        self.config = MarketConfig(tick_size=self.tick_size)
        ...
        FeedClass = FEEDS.get(self.exchange)
    ...
    def start_snapshot_loop(self): # FOR REACT
